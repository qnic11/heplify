...
func createFlags() {
	...
	flag.StringVar(&config.Cfg.TLSCertFile, "tls-cert-file", "", "client TLS certificate file path")
	flag.StringVar(&config.Cfg.TLSKeyFile, "tls-key-file", "", "client TLS key file path")
	flag.StringVar(&config.Cfg.TLSClientCAFile, "tls-client-ca-file", "", "TLS CA bundle for verifying server certificates")
	flag.StringVar(&config.Cfg.TLSMinVersion, "tls-min-version", "1.2", "minimum TLS version to use [1.0, 1.1, 1.2, 1.3]")
